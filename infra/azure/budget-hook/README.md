...
- `WATTRACKER_BUDGET_HOOK_TOKEN`: the deployment-only callback token.
...
the complete function URL with its `code` query parameter. The Functions
host validates that key before the ASGI app receives the request; an
`x-functions-key` header is the equivalent form. Treat the URL as a secret
and rotate the Function key together with the Bicep host/key inputs.
