...
def _days_ahead(*offsets):
    """ISO dates a few days from today (UTC).

    Every test that exports has to use dates near today: the manifest prunes
    uncompleted workouts more than exporter.EXPORT_GRACE_DAYS days old, so
    fixed calendar dates would stop exporting once they aged past the window.
    """
    return [(utc_today() + _dt.timedelta(days=n)).isoformat() for n in offsets]


def _fname(date, name="Threshold Intervals"):
...
def test_export_all_skips_completed(user_id, home_dir):
    ...
    out.mkdir()
    db.save_user_settings(user_id, {"workouts_dir": str(out), "zwift_id": "123"})
    dates = _days_ahead(1, 2)
    _plan_id, ids = _plan_with_workouts(user_id, dates)
    db.mark_plan_workout_completed(user_id, ids[0], 999, dates[0])
    res = exporter.sync_plan_exports(user_id)
    assert res["exported"] == 1
    files = os.listdir(out)
    assert files == [_fname(dates[1])]
...
def test_export_excludes_and_removes_ooto_workouts(user_id, home_dir):
    ...
    out.mkdir()
    db.save_user_settings(user_id, {"workouts_dir": str(out), "zwift_id": "123"})
    dates = _days_ahead(1, 6, 7)
    _plan_with_workouts(user_id, dates)
    exporter.sync_plan_exports(user_id)  # all 3 written
    assert len(os.listdir(out)) == 3
    # OOTO covers the last two days -> those .zwo removed on next sync.
    db.add_ooto_range(user_id, dates[1], dates[2])
    res = exporter.sync_plan_exports(user_id)
    assert res["exported"] == 1 and res["removed"] == 2
    assert os.listdir(out) == [_fname(dates[0])]
...
def test_export_all_route_and_ooto_routes(client, home_dir):
    ...
    r = client.post("/ooto/add", data={"start_date": dates[1], "end_date": ""},
                    follow_redirects=False)
    assert r.status_code == 303
    assert db.list_ooto_ranges(uid)[0]["start_date"] == dates[1]
    assert os.listdir(out) == [_fname(dates[0])]
