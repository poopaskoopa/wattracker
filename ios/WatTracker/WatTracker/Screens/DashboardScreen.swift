...
struct DashboardScreen: View {
    ...
    ///
    /// This screen used to build its own with `makeSession()`. That predates
    /// the gate: with two sessions over one keychain credential, a sign-out or
...
final class DashboardModel {
    ...
    /// supplied. That predates `SessionGate`, which now owns the single
    /// session over the single keychain credential. Two sessions over one
